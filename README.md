...
**No active warnings? Defaults to today's extended forecast instead** — BOM's fuller extended-forecast paragraph for today (e.g. "Cloudy. Medium chance of showers, most likely later in the day..."), not just the short outlook phrase, with no red border. The footer is never empty; it always shows *something* useful, whether that's an active warning or today's fuller forecast text.
...
- A **WeatherFlow Tempest** weather station, exposed via the **[`weatherflow2mqtt`](https://github.com/briis/hass-weatherflow2mqtt)** add-on — **not** Home Assistant Core's official cloud-based WeatherFlow integration, which only exposes raw device fields and is missing several derived sensors this card reads directly (feels-like temperature, a rain intensity descriptor, lightning strikes in the last hour, average wind bearing/speed, etc.). The upstream add-on is archived/unmaintained (last real commit 2023, marked no-longer-maintained in 2024) — this card was built and tested against **[a maintained fork](https://github.com/Vatoe/hass-weatherflow2mqtt)**, recommended if you're installing fresh. **Already running the old upstream add-on? Switching is recommended** — entity IDs are unchanged (a drop-in replacement, no downstream config to fix — verified live on a real migration), and the fork carries several fixes upstream never got:
  - A root-caused fix for the "Could not write to High and Low Table" crash — a stored `NULL` max/min value threw an unguarded `TypeError` that silently aborted *every* sensor's update for that cycle, not just the affected one; now self-healing, no manual database changes needed
  - A separate crash reading an empty sensor-storage table (`UnboundLocalError` in `readStorage()`) fixed the same way
  - Parameterized SQL (was interpolating live sensor values directly into query text) in both the High/Low update and the pressure-writing path, with NaN/Infinity guards added
  - A full automated test suite + CI running on every push — upstream had neither
  - Current, CVE-patched dependencies (`paho-mqtt`, `aiohttp`, `pytest`, `pyweatherflowudp`, `PyYAML`, `pytz`) — upstream's pinned versions include multiple since-disclosed CVEs
  - A Dockerfile base-image fix (`buster` → `bookworm`) — upstream's own image can no longer build at all, since Debian buster's package repos have been taken offline

  **Don't have it yet?** Settings → Add-ons → Add-on Store → ⋮ (top right) → Repositories → add `https://github.com/Vatoe/hass-weatherflow2mqtt` → find **WeatherFlow to MQTT (Fixed Fork)** in the store and install. Needs an MQTT broker add-on already running (e.g. Mosquitto — auto-detected if it's the official Home Assistant Community Add-ons one). `STATION_ID`/`STATION_TOKEN` (from [tempestwx.com](https://tempestwx.com/settings/tokens)) are optional — only needed for cloud forecast data, not for the local UDP sensor readings this card relies on. Full option reference: the fork's [`DOCS.md`](https://github.com/Vatoe/hass-weatherflow2mqtt/blob/main/DOCS.md)
...
- Feels **colder**: actual 23–25°C needs ≥4°C gap; 19–23°C needs >4°C; 15–19°C needs ≥3°C; ≤15°C needs only ≥1°C.
